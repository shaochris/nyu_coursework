...
void printState(stateStruct state, int cycle)
{
    ...
    if (printstate.is_open())
    {
        ...
        else
        {
            ...
            if ("beq" == state.EX.INS)
            {
                printstate<<"EX.Read_data1:\t"<<state.EX.Read_data1<<endl; // struct name incomplete
                printstate<<"EX.Read_data2:\t"<<state.EX.Read_data2<<endl; // 
                printstate<<"EX.Imm:\t"<<state.EX.Imm<<endl; // 
                printstate<<"EX.Rs:\t"<<"X"<<endl;
                printstate<<"EX.Rt:\t"<<"X"<<endl;
            ...
            {   
                ...
                if(state.EX.INS == "beq")
                {
                    ...
                    if (state.EX.Read_data1 != state.EX.Read_data2)
                    {
                        ...
                        newState.IF.PC = state.IF.PC.to_ulong() -4 +  bitset<30>(sign_extend(newState.EX.Imm).to_string().substr(2,30)).to_ulong()*4;
                        newState.IF.nop = 0;
